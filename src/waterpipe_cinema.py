...
@gif.frame
def gif_streamlines(x_wind, y_wind, level, time_slice=-1):
...
frames = []
for level in range(38):
    frame = gif_streamlines(x_wind, y_wind, level)
    frames.append(frame)

gif.save(frames, str(savepath) + '/streamlines.gif', duration = 19, unit = 's', between='startend')
