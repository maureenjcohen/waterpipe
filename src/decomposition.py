...
def decomposition(cubes, n=3, start=0, end=120, level=47, qscale=5):  
    ...
    q1 = ax1.quiver(X[::n,::n], Y[::n,::n], np.roll(x_wind[level,::n,::n].data, 72, axis=1), np.roll(y_wind[level,::n,::n].data, 72, axis=1), angles='xy', scale_units='xy', scale=qscale)
    # Create a quiver plot. The np.roll function moves the cube data so the substellar point is centred.
    ax1.quiverkey(q1, X=0.9, Y=1.05, U=qscale, label='%s m/s' %qscale, labelpos='E', coordinates='axes')
    ...
    q2 = ax2.quiver(X[::n,::n], Y[::n,::n], np.roll(uchi[level,::n,::n].data, 72, axis=1), np.roll(-vchi[level,::n,::n].data, 72, axis=1), angles='xy', scale_units='xy', scale=qscale)
    # Note -vchi is plotted. Same with the other y components below.
    ax2.quiverkey(q2, X=0.9, Y=1.05, U=qscale, label='%s m/s' %qscale, labelpos='E', coordinates='axes')
    ...
    q3 = ax3.quiver(X[::n,::n], Y[::n,::n], np.roll(upsi[level,::n,::n].data, 72, axis=1), np.roll(-vpsi[level,::n,::n].data, 72, axis=1), angles='xy', scale_units='xy', scale=qscale)
    ax3.quiverkey(q3, X=0.9, Y=1.05, U=qscale, label='%s m/s' %qscale, labelpos='E', coordinates='axes')
    ...
    q4 = ax4.quiver(X[::n,::n], Y[::n,::n], np.roll(eddy_upsi[level,::n,::n].data, 72, axis=1), np.roll(-eddy_vpsi[level,::n,::n].data, 72, axis=1), angles='xy', scale_units='xy', scale=qscale)
    ax4.quiverkey(q4, X=0.9, Y=1.05, U=qscale, label='%s m/s' %qscale, labelpos='E', coordinates='axes')
